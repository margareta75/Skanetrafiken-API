...
public class Servlet extends HttpServlet {
	...
	private int idFetch(NodeList node, String name) {
		...
		int idNumber = 0;
		for (int i = 0; i < nList.getLength(); i++) { // g�r i genom alla points en efter en tills man hittar r�tt
														// point!

			NodeList pointNode = nList.item(i).getChildNodes();

			for (int j = 0; j < pointNode.getLength(); j++) { // Ska hittar r�tt ID och name
